<script>
	...
	console.log('ready envio interes con el verifica');
	...
	async function enviarDte(codigoGeneracion, numeroControl) {
		...
		if(dataDte == null) {
			return;
              ...
              GuardaJson(documento,dataDte,codigoGeneracion,numeroControl);
	  ...
	  $.ajax({
		  ...
		  success: function(dataRespuesta) {
			  // console.log(data);
               ...
               if(dataRespuesta.estado=='PROCESADO' || dataRespuesta.selloRecibido!=''){
                   actualizarEstadoDte(1,numeroControl,codigoGeneracion);
                   mensaje('success', 'Documento Guardado correctamente', 'success');
           } 
              
              if (dataRespuesta.estado=='RECHAZADO' || dataRespuesta.selloRecibido==''){
               actualizarEstadoDte(6,numeroControl,codigoGeneracion);
               mensaje('info', 'Pasando DTE a Revision', 'success');
           }
              guardarBitacoraFactura(dataRespuesta,'s');
              recepcionmh=dataRespuesta.selloRecibido;
		 ...
		 // error: function(dataRespuesta) {
          ...
          mensaje('error', 'Ministerio Excedio Tiempo de respuesta', 'error');
            actualizarEstadoDte(3,numeroControl,codigoGeneracion);
        } else{
           ...
           // res.JSON.parse(
            ...
            actualizarEstadoDte(6,numeroControl,codigoGeneracion);
            mensaje('info', 'Pasando DTE a Revision Error de Recepcion', 'success');
    ...
    /**/
    function actualizarEstadoDte(estado,numeroControl,codigoGeneracion){
    ...
    function GuardaJson(jsonDtef,jsonDte,codigoGeneracion,numeroControl){
