...
class ventatercero extends CI_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model( "mhdte/ventatercero_model" );
    ...
    public function store() {
        ...
        $nitPorCuenta = $this->input->post( "item70" );
        $nomDenominacion = $this->input->post( "item71" );
        ...
        $data  = array(
            ...
            'nitPorCuenta'=>$nitPorCuenta,
            'nomDenominacion'=>$nomDenominacion

        );

        if ( $this->ventatercero_model->save( $data ) ) {
