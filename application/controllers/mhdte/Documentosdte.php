...
class documentosdte extends CI_Controller
{
	...
	public function modificarEstadoDte(){
		...
		$response = $this->json_model->modificarEstadoDte($numeroControl, $codigoGeneracion, $data);
        echo json_encode($response);
	}
    
    public function actualizarEstadoDteCon(){
	
    
		$estado = $this->input->post('estado');
		$lista = $this->input->post('codigoGeneracion');
        $data = array(
            'estado' => $estado,
        );
        if($lista!=''){
        ...
        for ( $i = 0; $i<count( $lista ); $i++ ) {
            echo json_encode($lista[$i]['numeroControl']);
        $response = $this->json_model->modificarEstadoDte($lista[$i]['numeroControl'],$lista[$i]['codigoGeneracion'], $data);
        }
