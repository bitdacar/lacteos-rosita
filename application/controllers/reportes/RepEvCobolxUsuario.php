...
class RepEvCobolxUsuario extends CI_Controller {
    ...
    public function RepEvCobolxUsuario() {
 ...
 sleep(2);
        ...
        $respuesta=$this->integracion_model->ConsultasXusuarios($fechini,$fechfin,$area,substr($text, 1),$id);
           if($area=='00'){
             $txt=" Usuarios Generales";
        }else{
         $vendedor=$this->integracion_model->vendedor($area);
            $txt=$vendedor[0]->cNombre;
        }
        ...
        $pdf->Ln( 4 );
        $pdf->Cell( 35 );
        $pdf->Cell( 175, 2, utf8_decode( strtoupper( "REPORTE POR Usuario: ".$txt."  Desde: ".date('d/m/Y', strtotime($fechini) )." Hasta: ".date('d/m/Y', strtotime($fechfin) ) ) ), 0, 0, 'C' );
            ...
            if(!empty($repExtras)){
              //  echo json_encode($repExtras);echo "<br>";
                $listasOrder[]=$data  = array(
                    ...
                    'valor' => $repExtras[0]->montoPorFormaPag,
                    ...
                    'totalGravada' => $repExtras[0]->totalGravada ,
                );
            }
